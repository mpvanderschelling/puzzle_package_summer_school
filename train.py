...
class ABCTrainer(ABC):
    """
    Trainer that does everything except policy update.
    """
    def __init__(self,
    ...
    def interact(self, num_steps: int):
        ...
        for step in range(num_steps):
            ...
            self.exp_buffer[episode_num]['obs'].append(obs.cpu())     # Store on CPU to save GPU memory
            self.exp_buffer[episode_num]['action'].append(action.cpu()) # (batch_size,)
            self.exp_buffer[episode_num]['logprob'].append(logprob.cpu()) # (batch_size,)
            self.exp_buffer[episode_num]['reward'].append(reward.cpu()) # (batch_size,)
...
class VanillaReinforceTrainer(ABCTrainer):
    """
    ...
    def __init__(self,
                 ...
                 jr_key: int = 37,
                 device: str = "cpu"):
        super().__init__(env, eval_env, agent, optimizer, optimizer_kwargs, jr_key, device)
    ...
    def get_rewards_to_go(self):
        ...
        for episode_num in self.exp_buffer.keys():
            ...
            rewards = torch.stack(self.exp_buffer[episode_num]['reward']).to(self.device)
    ...
    def policy_update(self):
        ...
        for episode_num in self.exp_buffer.keys():
            ...
            logprobs = torch.stack(self.exp_buffer[episode_num]['logprob']).squeeze(-1).to(self.device)
...
if __name__ == "__main__":
    ...
    agent = PolicyGradientAgent(
        ...
        channel_num_list=[16, 32, 64, 64], 
        kernel_size_list=[3]
    )
    
    trainer = VanillaReinforceTrainer(env, eval_env, agent, device=device)
    trainer.train(num_episodes=300, max_steps=trainer.n_pieces * 2, eval_interval=5)
